...
@group(1) @binding(1) var texRayStart: texture_2d<f32>;
@group(1) @binding(2) var texRayEnd: texture_2d<f32>;
@group(1) @binding(3) var samplerLin: sampler;
@group(1) @binding(4) var tfCtOpacity: texture_1d<f32>;
@group(1) @binding(5) var samplerNN: sampler;
...
@group(2) @binding(1) var<uniform> stepsCount: i32;
...
fn ComputeGradient(position: vec3<f32>, step: f32) -> vec3f
{
	...
	result.x = textureSample(textMain, samplerLin, position + dirs[0] * step).a - textureSample(textMain, samplerLin, position - dirs[0] * step).a;
	result.y = textureSample(textMain, samplerLin, position + dirs[1] * step).a - textureSample(textMain, samplerLin, position - dirs[1] * step).a;
	result.z = textureSample(textMain, samplerLin, position + dirs[2] * step).a - textureSample(textMain, samplerLin, position - dirs[2] * step).a;
	let l = length(result);
	if l == 0.0
	{
		return vec3f(0.0);
...
*/
fn GetNumberOfSamples(ray_length: f32, step_size: f32) -> i32
{
